...
const loadBackupData = async ({ accessToken, userId }: LoadBackupDataRequestArgsType): Promise<void> => {
  const { data }: AxiosResponse = await axios.post(`${API_BASE_URL}${LOAD_BACKUP_DATA_ROUTE}`, {
    accessToken,
    userId,
  });
...
export default function useLoadBackupData() {
  return useMutation((args: LoadBackupDataRequestArgsType) => loadBackupData(args));
}
